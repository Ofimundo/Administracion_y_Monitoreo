...
exports.INITIAL_DATABASE = void 0;
exports.getSimulatedDatabase = getSimulatedDatabase;
exports.saveSimulatedDatabase = saveSimulatedDatabase;
exports.isWeekendOrHoliday = isWeekendOrHoliday;
exports.getPreviousWorkDay = getPreviousWorkDay;
exports.PA_INS_BITACORA_ACEPTACION_RECHAZO = PA_INS_BITACORA_ACEPTACION_RECHAZO;
exports.PA_INS_ONBOARDING_ACEPTACION_RECHAZO = PA_INS_ONBOARDING_ACEPTACION_RECHAZO;
exports.PA_UPD_ESTADO_ACEPTACION_RECHAZO = PA_UPD_ESTADO_ACEPTACION_RECHAZO;
exports.PA_UPD_BITACORA_ACEPTACION_RECHAZO = PA_UPD_BITACORA_ACEPTACION_RECHAZO;
exports.PA_SEL_INDICADORES = PA_SEL_INDICADORES;
exports.PA_CORREO_ACEPTACION_RECHAZO = PA_CORREO_ACEPTACION_RECHAZO;
exports.PA_SII_ACEPTACION_RECHAZO = PA_SII_ACEPTACION_RECHAZO;
exports.PA_NOTAS_CREDITO_ACEPTACION_RECHAZO = PA_NOTAS_CREDITO_ACEPTACION_RECHAZO;
exports.PA_EJECUCION_ACEPTACION_RECHAZO = PA_EJECUCION_ACEPTACION_RECHAZO;
// lib/db-simulation.ts
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
// RUTA DE PERSISTENCIA LOCAL
const SIMULATED_DB_FILE = fs_1.default.existsSync(path_1.default.join(__dirname, "simulated-db.json"))
    ? path_1.default.join(__dirname, "simulated-db.json")
    : path_1.default.join(__dirname, "..", "src", "simulated-db.json");
...
function getSimulatedDatabase() {
    try {
        if (fs_1.default.existsSync(SIMULATED_DB_FILE)) {
            const data = fs_1.default.readFileSync(SIMULATED_DB_FILE, "utf-8");
    ...
    saveSimulatedDatabase(exports.INITIAL_DATABASE);
    return exports.INITIAL_DATABASE;
}
function saveSimulatedDatabase(db) {
    try {
        // Asegurarse de que el directorio existe
        const dir = path_1.default.dirname(SIMULATED_DB_FILE);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        fs_1.default.writeFileSync(SIMULATED_DB_FILE, JSON.stringify(db, null, 2), "utf-8");
        return true;
    }
    catch (error) {
...
function isWeekendOrHoliday(dateStr, feriados) {
...
function getPreviousWorkDay(dateStr, feriados) {
 ...
 */
function PA_INS_BITACORA_ACEPTACION_RECHAZO(db, params) {
    const nowStr = new Date().toISOString();
    const existingIdx = db.bitacora.findIndex((b) => b.folio_documento === params.folio && b.tipo_documento === params.tipo_documento);
    const entry = {
 ...
 */
function PA_INS_ONBOARDING_ACEPTACION_RECHAZO(db, params) {
    const existing = db.bitacora.find((b) => b.folio_documento === params.folio && b.tipo_documento === params.tipo_documento);
    if (existing)
        return db; // Ya existe, no se hace nada
 ...
 */
function PA_UPD_ESTADO_ACEPTACION_RECHAZO(db) {
    let updatedCount = 0;
    db.bitacora = db.bitacora.map((entry) => {
        if (entry.estado === "Pendiente" &&
            ...
            if (attr && attr.Atributo18) {
                ...
                updatedCount++;
                const docIdx = db.dte_doccab.findIndex((d) => d.Folio === entry.folio_documento && d.TipoDTE === entry.tipo_documento);
                if (docIdx >= 0) {
 ...
 */
function PA_UPD_BITACORA_ACEPTACION_RECHAZO(db) {
    let syncedCount = 0;
    db.bitacora = db.bitacora.map((entry) => {
        // Si estaba pendiente o manual en la bitácora
        if (entry.estado === "Pendiente" || entry.estado === "Manual" || entry.estado === "Pendiente Espera") {
            const doc = db.dte_doccab.find((d) => d.Folio === entry.folio_documento && d.TipoDTE === entry.tipo_documento);
            ...
            if (doc && (doc.EnProceso === 1 || doc.EnProceso === 2)) {
                ...
                return {
                    ...
                    motivo: `Sincronizado: Gestionado manualmente en Softland (Estado: ${doc.EnProceso === 1 ? "Aprobado" : "Rechazado"})`,
                    fecha_modificacion: new Date().toISOString(),
                };
            }
        }
 ...
 */
function PA_SEL_INDICADORES(db, params) {
...
function PA_CORREO_ACEPTACION_RECHAZO(params) {
 ...
 */
async function PA_SII_ACEPTACION_RECHAZO(db, params) {
    ...
    const docIdx = db.dte_doccab.findIndex((d) => d.Folio === params.folio && d.TipoDTE === params.tipoDocumento);
    if (docIdx >= 0) {
 ...
 */
function PA_NOTAS_CREDITO_ACEPTACION_RECHAZO(db, params) {
...
async function PA_EJECUCION_ACEPTACION_RECHAZO() {
    const startTime = new Date();
    const db = getSimulatedDatabase();
    const logs = [];
    const sentMails = [];
    const addLog = (message, type = "info", folio) => {
        logs.push({
            timestamp: new Date().toISOString(),
            message,
            type,
