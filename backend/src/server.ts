...
app.get("/api/sgc/stats", async (req, res) => {
  try {
    ...
    const result = await executeQuery(sgcQuery);
    ...
    const picking = result.recordset?.filter((r: any) => r.tipo_de_venta?.toLowerCase() === "picking").length || 0;
    const od = result.recordset?.filter((r: any) => r.tipo_de_venta?.toLowerCase() === "od").length || 0;
