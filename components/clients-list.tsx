...
import { clients, getClientServices, type Client } from "@/lib/services-data";
...
export function ClientsList({ onSelectClient }: ClientsListProps) {
  ...
  const exportToExcel = () => {
    const excelData = filteredClients.map(client => ({
      ...
      "Porcentaje de Error": `${client.errorPercentage}%`,
      "Estado": client.status === "success" ? "Excelente" : client.status === "warning" ? "Warning" : "Crítico",
      "Servicios Contratados": getClientServices(client.id).length,
    }));
    ...
    <div className="space-y-6">
              ...
              <div>
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="warning">⚠️ Warning (1-10% error)</SelectItem>
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap gap-2 items-center p-3 bg-muted/30 rounded-lg">
          ...
          {statusFilter !== "all" && (
            <Badge variant="secondary" className="gap-1">
              {statusFilter === "success" ? "✅ Excelente" : statusFilter === "warning" ? "⚠️ Warning" : "❌ Crítico"}
            </Badge>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredClients.map((client) => {
          ...
          const successRate = 100 - client.errorPercentage;
              ...
              <CardContent className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  ...
                  <div className="bg-muted/50 rounded-lg p-2 text-center">
                    <p className={cn(
                      "text-2xl font-bold",
                      client.errorPercentage === 0 ? "text-green-600" :
                      client.errorPercentage <= 10 ? "text-yellow-600" : "text-red-600"
                    )}>
                      {client.errorPercentage}%
                    </p>
